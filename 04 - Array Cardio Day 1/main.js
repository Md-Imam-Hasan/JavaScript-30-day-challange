// Get your shorts on - this is an array workout!
    ...
    const totalAge = inventors.reduce((sum,inventor)=>sum+(inventor.passed-inventor.year),0);
    console.log(totalAge);

    // 5. Sort the inventors by years lived
    const sortByYearLived = inventors.sort((a,b) => (b.passed-b.year)-(a.passed-a.year));
    console.table(sortByYearLived);
    console.log(sortByYearLived);

    // 6. create a list of Boulevards in Paris that contain 'de' anywhere in the name
    // https://en.wikipedia.org/wiki/Category:Boulevards_in_Paris
    // const category = document.querySelector('.mw-category');
    // const link = [...category.querySelectorAll('a')];
    // const de = link.map(name=>name.textContent).filter(city=>city.includes('de'))
    // console.log(de);

    // 7. sort Exercise
    // Sort the people alphabetically by last name
    const sortPeople = people.sort((lastname,nextname)=> {
    ...
    const data = ['car', 'car', 'truck', 'truck', 'bike', 'walk', 'car', 'van', 'bike', 'walk', 'car', 'van', 'car', 'truck' ,'ok'];
